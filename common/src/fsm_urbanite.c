...
static bool check_pause_display(fsm_t *p_this)
{
   ...
   return (duracion > 0 && duracion < p_fsm_urbanite_t->on_off_press_time_ms && duracion > p_fsm_urbanite_t->pause_display_time_ms);
...
static bool check_activity(fsm_t *p_this)
{
   ...
   return ( // accedemos al boton, ultras y display desde el urbanite
       fsm_button_check_activity(p_fsm_urbanite_t->p_fsm_button) || fsm_ultrasound_check_activity(p_fsm_urbanite_t->p_fsm_ultrasound_rear) || fsm_display_check_activity(p_fsm_urbanite_t->p_fsm_display_rear));
}
...
static void do_start_up_measure(fsm_t *p_this)
{
   ...
   fsm_ultrasound_start(p_fsm_urbanite_t->p_fsm_ultrasound_rear);//TODO esto quizás falla
...
static void do_display_distance(fsm_t *p_this)
{
   ...
   // TODO uint32_t distancia = fsm_ultrasound_get_distance(p_fsm_urbanite_t->p_fsm_ultrasound_rear);
   // lo guardo en una variable para no estar llamando a la funcion constantemente

   if (p_fsm_urbanite_t->is_paused)
   {

      if (fsm_ultrasound_get_distance(p_fsm_urbanite_t->p_fsm_ultrasound_rear) < WARNING_MIN_CM / 2)
      {
         fsm_display_set_distance(p_fsm_urbanite_t->p_fsm_display_rear, fsm_ultrasound_get_distance(p_fsm_urbanite_t->p_fsm_ultrasound_rear));
   ...
   printf("[URBANITE][%ld] Distance: %ld cm\n", port_system_get_millis(), fsm_ultrasound_get_distance(p_fsm_urbanite_t->p_fsm_ultrasound_rear));
}
...
static fsm_trans_t fsm_trans_urbanite[] = {
    ...
    {-1, NULL, -1, NULL}}; /*!< Array representing the transitions table of the FSM Urbanite.*/
