...
static bool check_on (fsm_t * p_this){
    ...
    return p_fsm->status;//luego se comrpueba la flag
}
...
static bool check_off	(	fsm_t * 	p_this	){
    return !check_on(p_this); //llama a la funcion anterior
}
/**
...
static bool check_new_measurement	(	fsm_t * 	p_this	){
    fsm_ultrasound_t *p_fsm = (fsm_ultrasound_t *)p_this; //primero se hace el cast
    return p_fsm->new_measurement; //se obtiene el tiempo del tick
}
 ...
 * @param p_this 
 */
static void do_set_distance(fsm_t *p_this) {//TODO AQUI VIENE LA GORDA
