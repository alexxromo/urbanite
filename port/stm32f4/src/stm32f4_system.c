...
#define IDR5_MASK 0x20 /*!< En binario 0b0010 0000 */

bool 	stm32f4_system_gpio_read (GPIO_TypeDef *p_port, uint8_t pin){
  return (bool)((p_port-> IDR & BIT_POS_TO_MASK(pin)) !=0);
}


//TODO
#define BSRR5_MASK 0x20 /*!< En binario 0b0010 0000 */

void stm32f4_system_gpio_write (GPIO_TypeDef *p_port, uint8_t pin, bool value){
  if (value) {
    p_port->BSRR = BIT_POS_TO_MASK(pin);  // Se escribe un 1 en el bit correspondiente
  } else {
    p_port->BSRR = BIT_POS_TO_MASK(pin) << 16;  // se resetea
}
}


void 	stm32f4_system_gpio_toggle (GPIO_TypeDef *p_port, uint8_t pin){
  bool estado = stm32f4_system_gpio_read(p_port, pin);  // lee el estado usando la funcion read
    stm32f4_system_gpio_write(p_port, pin, !estado);    //lo cambia
}
...
void port_system_sleep (void){
  port_system_systick_suspend(); //suspend the count of the SysTick.
  port_system_power_sleep(); //enter sleep in sleep mode.
}